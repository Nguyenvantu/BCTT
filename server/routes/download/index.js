...
router.get('/song/:songTitle/:id', (req, res, next) => {
  ...
  co(function* () {
    ...
    const regex = /media\/get-source\?type=(audio|video)&key=(.{33})/; // get the resouce url
    const match = html.match(regex);
    if (!match) throw new Error("can't find the resource URL");

    const resource = yield request(`https://mp3.zing.vn/xhr/media/get-source?type=audio&key=${match[2]}`);
    const data = JSON.parse(resource).data;
    const songURI = `https:${data.source['128']}`;
    // console.log(songURI);
